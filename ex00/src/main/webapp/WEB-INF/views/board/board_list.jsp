	...
	<table border="1">
		...
		<c:if test="${!empty blist }">
			...
			<tr>
				...
				<strong>[댓글개수] ${b.replycnt}</strong><%--nbsp;는 한칸의 빈공백을 만들어 준다. --%></a></th>
				<%-- board_cont?bno=번호값 형태의 웹 주소창에 노출되는 get방식으로 bno파라미터이름에
				게시판 번호값을 담아서 내용보기 매핑주소로 전달. --%>
				<th>${b.writer }</th>
				<th>${b.viewcnt }</th>
				<th>${b.regdate }</th>
			</tr>
		...
		<tr>
			...
			<%-- 자바 스크립트 location에 의해서 게시판 글쓰기 폼으로 이동 --%>
...
<script>
	...
	//자바스크립트에서 스프링 컨트롤러의 키 이름을 참조해서 EL or JSTL로 가져올 수 있다.
	//표현 언어인 el로 키를 참조해서 SUCCESS 문자열을 가져옴
	//jQuery로 표현하면 $msg(jQuery)='${msg}(el)'; 여기서 msg는 jQuery 변수명이다.
	if (msg == "SUCCESS") {
